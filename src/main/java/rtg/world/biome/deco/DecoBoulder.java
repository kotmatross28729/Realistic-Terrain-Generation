        ...
        if (this.allowed) {
            ...
            for (int l1 = 0; l1 < this.strengthFactor * strength; ++l1) {
                ...
                if (k1 >= this.minY && k1 <= this.maxY && rand.nextInt(this.chance) == 0) {
                    ...
                    // If we're in a village, check to make sure the boulder has extra room to grow to avoid corrupting
                    // the village.
                    if (hasPlacedVillageBlocks) {
                        if (!worldUtil
                            .isSurroundedByBlock(Blocks.air, 2, SurroundCheckType.CARDINAL, rand, i1, k1, j1)) {
                            return;
                        }
                    }
