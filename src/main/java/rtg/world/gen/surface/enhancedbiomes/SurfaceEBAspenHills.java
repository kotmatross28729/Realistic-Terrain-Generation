...
public class SurfaceEBAspenHills extends SurfaceEBBase {
    ...
    public SurfaceEBAspenHills(BiomeConfig config, Block top, byte topByte, Block fill, byte fillByte, boolean genBeach,
        Block genBeachBlock, float minCliff) {
    ...
    public SurfaceEBAspenHills(BiomeConfig config, Block top, byte topMeta, Block fill, byte fillMeta, boolean genBeach,
        Block genBeachBlock, float minCliff, float stoneCliff, float stoneHeight, float stoneStrength, float snowCliff,
        float snowHeight, float snowStrength, float clayCliff) {
                    ...
                    if (cliff == 1) {
                        if (rand.nextInt(3) == 0) {
                            ...
                            metadata[(y * 16 + x) * 256 + k] = EBAPI
                                .ebStonify(EBAPI.SLATE, hcCobbleMeta(world, i, j, x, y, k));
                        } else {

                            blocks[(y * 16 + x) * 256 + k] = EBAPI
                                .ebStonify(EnhancedBiomesBlocks.stoneEB, hcStone(world, i, j, x, y, k));
                            metadata[(y * 16 + x) * 256 + k] = EBAPI
                                .ebStonify(EBAPI.SLATE, hcStoneMeta(world, i, j, x, y, k));
                        }
                    } else if (cliff == 2) {
