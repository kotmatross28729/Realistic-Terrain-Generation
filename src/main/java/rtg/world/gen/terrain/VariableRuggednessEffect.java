...
public class VariableRuggednessEffect extends HeightEffect {
    ...
    public VariableRuggednessEffect(HeightEffect smoothTerrain, HeightEffect ruggedTerrain, float startTransition,
        float transitionWidth, float wavelength) {
    ...
    public final float added(OpenSimplexNoise simplex, CellNoise cell, float x, float y) {
        float choice = simplex.octave(octave)
            .noise2((float) x / wavelength, (float) y / wavelength);
