...
public class WorldGenTreesRTG extends WorldGenTrees {
            ...
            if (!flag) {
                ...
                if (isSoil && worldY < 256 - l - 1) {
                    ...
                    for (k1 = 0; k1 < l; ++k1) {
                        ...
                        if (block.isAir(world, worldX, worldY + k1, worldZ)
                            || block.isLeaves(world, worldX, worldY + k1, worldZ)) {
                            this.setBlockAndNotifyAdequately(
                                world,
                                worldX,
                                worldY + k1,
                        ...
                        for (k1 = worldY - 3 + l; k1 <= worldY + l; ++k1) {
                            ...
                            for (i2 = worldX - l1; i2 <= worldX + l1; ++i2) {
                                for (j2 = worldZ - l1; j2 <= worldZ + l1; ++j2) {
                                    if (world.getBlock(i2, k1, j2)
                                        .isLeaves(world, i2, k1, j2)) {
                                        if (rand.nextInt(4) == 0 && world.getBlock(i2 - 1, k1, j2)
                                            .isAir(world, i2 - 1, k1, j2)) {
                                        ...
                                        if (rand.nextInt(4) == 0 && world.getBlock(i2, k1, j2 - 1)
                                            .isAir(world, i2, k1, j2 - 1)) {
                            ...
                            for (k1 = 0; k1 < 2; ++k1) {
                                for (i3 = 0; i3 < 4; ++i3) {
                                    if (rand.nextInt(4 - k1) == 0) {
                                        ...
                                        this.setBlockAndNotifyAdequately(
                                            world,
                                            worldX + Direction.offsetX[Direction.rotateOpposite[i3]],
                                            worldY + l - 5 + k1,
                                            worldZ + Direction.offsetZ[Direction.rotateOpposite[i3]],
                                            Blocks.cocoa,
                                            l1 << 2 | i3);
                                    }
                                }
    ...
    private void growVines(World world, int worldX, int worldY, int worldZ, int vineLength) {
        ...
        while (true) {
            ...
            if (!world.getBlock(worldX, worldY, worldZ)
                .isAir(world, worldX, worldY, worldZ) || i1 <= 0) {
                return;
            }
