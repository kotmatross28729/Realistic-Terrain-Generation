...
public class Trackers<Type> {
    ...
    private HashMap<Key, WeakReference<Acceptor<Type>>> trackers = new HashMap<Key, WeakReference<Acceptor<Type>>>();
    ...
    public void informOnChange(Acceptor<Type> tracker) {
        for (Key key : trackers.keySet()) {
            Acceptor<Type> existing = trackers.get(key)
                .get();
            if (tracker == existing) {
                return;
            }
    ...
    public void update(Type toUpdate) {
        ...
        for (Key key : trackers.keySet()) {
            Acceptor<Type> tracker = trackers.get(key)
                .get();
            if (tracker == null) {
                toDelete.add(key);
            } else {
