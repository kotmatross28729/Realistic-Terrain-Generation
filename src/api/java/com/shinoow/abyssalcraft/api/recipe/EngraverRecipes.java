...
public class EngraverRecipes {
    ...
    public ItemStack getEngravingResult(ItemStack par1ItemStack) {
        if (coins.contains(par1ItemStack)) return par1ItemStack;
    ...
    public ItemStack getEngravingResult(ItemStack par2, ItemEngraving par1) {
        for (ItemStack stack : coins) if (areStacksEqual(par2, stack))
            if (engravings.get(par1) != null) if (par2.getItem() != ACItems.coin && par1 == ACItems.blank_engraving
                || par2.getItem() == ACItems.coin && par1 != ACItems.blank_engraving) return engravings.get(par1);
    ...
    public float getExperience(ItemStack par1ItemStack) {
        float ret = par1ItemStack.getItem()
            .getSmeltingExperience(par1ItemStack);
        if (ret != -1) return ret;

        for (Entry<ItemStack, Float> entry : experienceList.entrySet())
            if (areStacksEqual(par1ItemStack, entry.getKey())) return entry.getValue()
                .floatValue();
