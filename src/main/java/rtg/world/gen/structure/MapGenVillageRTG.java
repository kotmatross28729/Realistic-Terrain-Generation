...
public class MapGenVillageRTG extends MapGenVillage {
    ...
    public MapGenVillageRTG() {
        ...
        if (minDistanceVillages < 1) {
            ...
            Logger.warn(
                "Config warning: Minimum distance between villages must be greater than 0. Reverting to vanilla value (%d).",
                minDistanceVillages);
        }
        if (maxDistanceVillages < 1) {
            maxDistanceVillages = 32;
            Logger.warn(
                "Config warning: Maximum distance between villages must be greater than 0. Reverting to vanilla value (%d).",
                maxDistanceVillages);
        }
        ...
        if (minDistanceVillages > maxDistanceVillages) {
            ...
            maxDistanceVillages = 32;
            Logger.warn(
                "Config warning: Minimum distance between villages must be less than maximum distance between villages. Reverting to vanilla values.");
        }
        // Bad things happen when min & max are the same.
        else if (minDistanceVillages == maxDistanceVillages) {
            maxDistanceVillages++;
            Logger.warn(
                "Config warning: Maximum distance between villages must be greater than minimum distance between villages. Increasing maximum to %d.",
                maxDistanceVillages);
        }
    ...
    protected boolean canSpawnStructureAtCoords(int par1, int par2) {
        boolean booRTGWorld = (worldObj.getWorldInfo()
            .getTerrainType() instanceof WorldTypeRTG) ? true : false;
        ...
        if (k == i1 && l == j1) {
            if (booRTGWorld && booRTGChunkManager) {
                ...
                canSpawnVillage = this.worldObj.getWorldChunkManager()
