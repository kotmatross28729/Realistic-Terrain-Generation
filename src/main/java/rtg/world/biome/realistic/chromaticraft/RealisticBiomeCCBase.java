...
public class RealisticBiomeCCBase extends RealisticBiomeBase {
    ...
    public RealisticBiomeCCBase(BiomeConfig config, BiomeGenBase b, BiomeGenBase riverbiome, TerrainBase t,
        SurfaceBase s) {
        ...
        if (Loader.isModLoaded("ChromatiCraft")) {
            ...
            for (int i = 0; i < 256; i++) {
                if (b[i] != null) {
                    ...
                    String biomeClass = ccBiome.getBiomeClass()
                        .getName();

                    if (biomeName == "Ender Forest" && biomeClass == "Reika.ChromatiCraft.World.BiomeEnderForest") {
                        ccEnderForest = new RealisticBiomeCCEnderForest(
                            ccBiome,
                            BiomeConfigCC.biomeConfigCCEnderForest);
                    } else if (biomeName == "Luminous Cliffs"
                        && biomeClass == "Reika.ChromatiCraft.World.BiomeGlowingCliffs") {
                            ccLuminousCliffs = new RealisticBiomeCCLuminousCliffs(
                                ccBiome,
                                BiomeConfigCC.biomeConfigCCLuminousCliffs);
                        } else if (biomeName == "Rainbow Forest"
                            && biomeClass == "Reika.ChromatiCraft.World.BiomeRainbowForest") {
                                ccRainbowForest = new RealisticBiomeCCRainbowForest(
                                    ccBiome,
                                    BiomeConfigCC.biomeConfigCCRainbowForest);
                            }
                }
            }
        }
