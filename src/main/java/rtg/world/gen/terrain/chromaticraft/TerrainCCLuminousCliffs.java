...
public class TerrainCCLuminousCliffs extends TerrainBase {
    ...
    public float generateNoise(OpenSimplexNoise simplex, CellNoise cell, int x, int y, float border, float river) {
        ...
        float secondAdded = simplex.octave(1)
            .noise2(x / width, y / width) * 10 + 40
        ...
        float thirdAdded = simplex.octave(2)
            .noise2(x / width, y / width) * 15 + 30
            + simplex.noise2(x / 20f, y / 20f) * 2f;
        /*
        ...
        float scatter = Math.abs(
            (float) simplex.octave(1)
                .noise(x / cragWidth, y / cragWidth, 1f));
