...
public class TreeRTGQuercusRobur extends TreeRTG {
     ...
     * logBlock, logMeta, leavesBlock, leavesMeta, <s>trunkSize</s>, <s>crownSize</s>, noLeaves<br>
     * <br>
        ...
        if (groundBlock != Blocks.grass && groundBlock != Blocks.dirt) {
            ...
            for (int h = this.basePos[1] + 1; h < (this.basePos[1] + this.heightLimit); h++) {
                ...
                if (checkBlock != Blocks.air && checkBlock != this.leavesBlock) {
                    Logger.debug(
                        "Invalid location (%d/%d/%d). Check block (%s) is not air or %s.",
                        this.basePos[0],
                        h,
                        this.basePos[2],
                        checkBlock.getLocalizedName(),
                        this.leavesBlock.getLocalizedName());
                    return false;
                }
            }
            ...
            // else
            // {
            // Logger.debug("Valid location! Height limit has been changed from %d to %d.", this.heightLimit,
            // blockLine);
