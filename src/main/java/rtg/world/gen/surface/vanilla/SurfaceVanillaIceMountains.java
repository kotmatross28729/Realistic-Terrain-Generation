...
public class SurfaceVanillaIceMountains extends SurfaceBase {
    ...
    public SurfaceVanillaIceMountains(BiomeConfig config, Block top, Block filler, Block mixTop, Block mixFill,
        Block cliff1, Block cliff2, float mixWidth, float mixHeight, float smallWidth, float smallStrength) {
        super(config, top, (byte) 0, filler, (byte) 0);

        mixBlockTop = this.getConfigBlock(config, BiomeConfigVanillaIceMountains.surfaceMixBlockId, mixTop);
        mixBlockTopMeta = this
            .getConfigBlockMeta(config, BiomeConfigVanillaIceMountains.surfaceMixBlockMetaId, (byte) 0);

        mixBlockFill = this.getConfigBlock(config, BiomeConfigVanillaIceMountains.surfaceMixFillerBlockId, mixFill);
        mixBlockFillMeta = this
            .getConfigBlockMeta(config, BiomeConfigVanillaIceMountains.surfaceMixFillerBlockMetaId, (byte) 0);
