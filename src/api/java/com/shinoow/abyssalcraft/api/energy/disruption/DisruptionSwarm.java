...
public class DisruptionSwarm extends DisruptionEntry {
    ...
    public void disrupt(World world, int x, int y, int z, List<EntityPlayer> players) {
        ...
        if (!world.isRemote) for (Class<? extends EntityLivingBase> clazz : entities) for (int i = 0; i < 4; i++) try {
            EntityLivingBase entity = clazz.getConstructor(World.class)
                .newInstance(world);
            entity.setLocationAndAngles(
                x + randomNum(world.rand),
                y + 1,
                z + randomNum(world.rand),
                entity.rotationYaw,
