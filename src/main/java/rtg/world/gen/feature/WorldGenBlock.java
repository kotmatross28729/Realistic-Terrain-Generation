...
public class WorldGenBlock extends WorldGenerator {
    ...
    public WorldGenBlock(Block placeBlock, byte placeBlockMeta, Block replaceBlock, byte replaceBlockMeta,
        Block adjacentBlock, byte adjacentBlockMeta, int minAdjacents) {
        super(false);
    ...
    public boolean generate(World world, Random rand, int x, int y, int z) {
        ...
        if (targetBlock != replaceBlock) {
            // Logger.debug("Target block (%s) does not equal Replace block (%s)", targetBlock.getLocalizedName(),
            // replaceBlock.getLocalizedName());
            return false;
        }

        if (!isAdjacent(world, x, y, z)) {
            // Logger.debug("Target block (%s) is not adjacent to %s", targetBlock.getLocalizedName(),
            // this.adjacentBlock.getLocalizedName());
            return false;
        }
