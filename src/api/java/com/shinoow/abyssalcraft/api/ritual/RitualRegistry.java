...
public class RitualRegistry {
    ...
    public void addDimensionToBookType(int dim, int bookType) {
        if (bookType <= 4 && bookType >= 0) if (dim != -1 && dim != 1) dimToBookType.put(dim, bookType);
    ...
    /**
     * Maps a dimension to a name, in order to display it in the Necronomicon if rituals can only be performed in said
     * dimension
     * 
     * @param dim  The Dimension ID
    ...
    public void addDimensionToName(int dim, String name) {
        if (dim != -1 && dim != 1) dimToName.put(dim, name);
        else FMLLog.log("RitualRegistry", Level.ERROR, "You're not allowed to register that Dimension ID: %d", dim);
    }

    /**
     * Maps a dimension to a book type, in order to specify dimensions where a ritual of that book type can be
     * performed,<br>
     * and maps it to a name, in order to display it in the Necronomicon if rituals can only be performed in said
     * dimension
     * 
     * @param dim      The Dimension ID
     * @param bookType The Necronomicon book type required
    ...
    public void registerRitual(NecronomiconRitual ritual) {
        if (ritual.getBookType() <= 4 && ritual.getBookType() >= 0) {
            ...
            do {
                ...
                compare = iter.next();
                if (ritual.getUnlocalizedName()
                    .equals(compare.getUnlocalizedName())) {
                    FMLLog.log(
                        "RitualRegistry",
                        Level.ERROR,
                        "Necronomicon Ritual already registered: %s",
                        ritual.getUnlocalizedName());
                    return;
                }
            } while (!ritual.getUnlocalizedName()
                .equals(compare.getUnlocalizedName()));
        } else FMLLog
            .log("RitualRegistry", Level.ERROR, "Necronomicon book type does not exist: %d", ritual.getBookType());
    }
    ...
    private boolean areRitualsSame(NecronomiconRitual ritual, int dimension, int bookType, ItemStack[] offerings,
        ...
        if (ritual.getDimension() == dimension || ritual.getDimension() == -1)
            if (ritual.getBookType() <= bookType) if (ritual.getOfferings() != null && offerings != null)
                if (areItemStackArraysEqual(ritual.getOfferings(), offerings))
                    if (ritual.getSacrifice() == null && sacrifice == null
                        || areObjectsEqual(sacrifice, ritual.getSacrifice())) return true;
        return false;
    }
    ...
    private boolean areItemStackArraysEqual(Object[] array1, ItemStack[] array2) {
        ...
        for (ItemStack item : array2) if (item != null) itemList.add(item);

        if (itemList.size() == compareList.size())
            for (ItemStack item : itemList) for (Object compare : compareList) if (areObjectsEqual(item, compare)) {
    ...
    public boolean areObjectsEqual(ItemStack stack, Object obj) {
        if (obj instanceof ItemStack) return areStacksEqual(stack, (ItemStack) obj);
        else if (obj instanceof Item) return areStacksEqual(stack, new ItemStack((Item) obj));
        else if (obj instanceof Block) return areStacksEqual(stack, new ItemStack((Block) obj));
        else if (obj instanceof String)
            for (ItemStack item : OreDictionary.getOres((String) obj)) return areStacksEqual(stack, item);
        return false;
    }
