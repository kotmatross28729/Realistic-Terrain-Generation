...
public class CraftingStack {
     ...
     * @param recipe The Items/Blocks/ItemStacks used to craft said Item/Block/ItemStack (has to contain 9 elements, all
     *               of them can be null)
     */
        ...
        if (output != null) {
            if (recipe != null) {
                if (recipe.length == 9) {
                    ...
                    for (int i = 0; i < 9; i++) this.recipe[i] = APIUtils.convertToStack(recipe[i]);
                } else throw new ArrayIndexOutOfBoundsException(
                    "The array must contain preciesly 9 elements, not " + recipe.length + "!");
    ...
    public CraftingStack(Object output) {
        if (output != null) {
            ...
            this.output = APIUtils.convertToStack(output);
            for (Object thing : CraftingManager.getInstance()
                .getRecipeList()) if (thing instanceof IRecipe) {
                    IRecipe recipe = (IRecipe) thing;
                    if (recipe.getRecipeOutput() != null && recipe.getRecipeOutput()
                        .isItemEqual(this.output)) {
                        if (recipe instanceof ShapedRecipes) for (int i = 0; i < recipe.getRecipeSize(); i++)
                            stuff[i] = ((ShapedRecipes) recipe).recipeItems[i];
                        if (recipe instanceof ShapelessRecipes) for (int i = 0; i < recipe.getRecipeSize(); i++)
                            stuff[i] = ((ShapelessRecipes) recipe).recipeItems.get(i);
                        if (recipe instanceof ShapedOreRecipe) for (int i = 0; i < recipe.getRecipeSize(); i++)
                            stuff[i] = ((ShapedOreRecipe) recipe).getInput()[i];
                        if (recipe instanceof ShapelessOreRecipe) for (int i = 0; i < recipe.getRecipeSize(); i++)
                            stuff[i] = ((ShapelessOreRecipe) recipe).getInput()
                                .get(i);
            ...
            for (int i = 0; i < 9; i++) recipe[i] = APIUtils.convertToStack(stuff[i]);
        }

        for (ItemStack stack : recipe)
            if (stack != null && stack.getItemDamage() == OreDictionary.WILDCARD_VALUE) stack.setItemDamage(0);
    }
