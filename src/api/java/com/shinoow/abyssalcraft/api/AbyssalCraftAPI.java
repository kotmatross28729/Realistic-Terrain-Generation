...
public class AbyssalCraftAPI {
    ...
    public static DamageSource coralium = new DamageSource("coralium").setDamageBypassesArmor()
        .setMagicDamage();
    public static DamageSource dread = new DamageSource("dread").setDamageBypassesArmor()
        .setMagicDamage();
    public static DamageSource antimatter = new DamageSource("antimatter").setDamageBypassesArmor()
        .setMagicDamage();
    ...
    public static ArmorMaterial abyssalniteArmor = EnumHelper
        .addArmorMaterial("Abyssalnite", 35, new int[] { 3, 6, 8, 3 }, 13);
    public static ArmorMaterial dreadedAbyssalniteArmor = EnumHelper
        .addArmorMaterial("Dread", 36, new int[] { 3, 6, 8, 3 }, 15);
    public static ArmorMaterial refinedCoraliumArmor = EnumHelper
        .addArmorMaterial("Coralium", 37, new int[] { 3, 6, 8, 3 }, 14);
    public static ArmorMaterial platedCoraliumArmor = EnumHelper
        .addArmorMaterial("CoraliumP", 55, new int[] { 4, 7, 9, 4 }, 14);
    public static ArmorMaterial depthsArmor = EnumHelper.addArmorMaterial("Depths", 33, new int[] { 3, 6, 8, 3 }, 25);
    public static ArmorMaterial dreadiumArmor = EnumHelper
        .addArmorMaterial("Dreadium", 40, new int[] { 3, 6, 8, 3 }, 15);
    public static ArmorMaterial dreadiumSamuraiArmor = EnumHelper
        .addArmorMaterial("DreadiumS", 45, new int[] { 3, 6, 8, 3 }, 20);
    public static ArmorMaterial ethaxiumArmor = EnumHelper
        .addArmorMaterial("Ethaxium", 50, new int[] { 3, 6, 8, 3 }, 25);
    ...
    public static void setInternalNDHandler(IInternalNecroDataHandler handler) {
        if (internalNDHandler.getClass()
            .getName()
            .equals(DummyNecroDataHandler.class.getName())
            && Loader.instance()
                .getLoaderState() == LoaderState.PREINITIALIZATION
            && Loader.instance()
                .activeModContainer()
                .getModId()
                .equals("abyssalcraft"))
    ...
    public static void initPotionReflection() {
        for (Field f : PotionHelper.class.getDeclaredFields()) try {
            if (f.getName()
                .equals("potionRequirements")
                || f.getName()
                    .equals("field_77927_l")) {
            ...
            if (f.getName()
                .equals("potionAmplifiers")
                || f.getName()
                    .equals("field_77928_m")) {
                f.setAccessible(true);
                try {
    ...
    public static void addCrystallization(Block input, ItemStack output1, ItemStack output2, float xp) {
        CrystallizerRecipes.instance()
            .crystallize(input, output1, output2, xp);
    }
    ...
    public static void addOreSmelting(String input, String output, float xp) {
        if (!OreDictionary.getOres(output)
            .isEmpty()) for (ItemStack stack : OreDictionary.getOres(input))
                FurnaceRecipes.smelting()
                    .func_151394_a(
                        stack,
                        OreDictionary.getOres(output)
                            .get(0),
                        xp);
    }
    ...
    public static void addCrystallization(String input, String output1, String output2, float xp) {
        if (!OreDictionary.getOres(output1)
            .isEmpty()
            && !OreDictionary.getOres(output2)
                .isEmpty())
            for (ItemStack stack : OreDictionary.getOres(input)) addCrystallization(
                stack,
                OreDictionary.getOres(output1)
                    .get(0),
                OreDictionary.getOres(output2)
                    .get(0),
                xp);
    ...
    public static void addCrystallization(String input, String output1, int out1, String output2, int out2, float xp) {
        if (!OreDictionary.getOres(output1)
            ...
            && !OreDictionary.getOres(output2)
                .isEmpty()) {
            ItemStack o1 = OreDictionary.getOres(output1)
                .get(0)
                .copy();
            o1.stackSize = out1;
            ItemStack o2 = OreDictionary.getOres(output2)
                .get(0)
                .copy();
            o2.stackSize = out2;
            for (ItemStack stack : OreDictionary.getOres(input)) addCrystallization(stack, o1, o2, xp);
        }
    }
    ...
    public static void addSingleCrystallization(String input, String output, float xp) {
        if (!OreDictionary.getOres(output)
            .isEmpty())
            for (ItemStack stack : OreDictionary.getOres(input)) addSingleCrystallization(
                stack,
                OreDictionary.getOres(output)
                    .get(0),
                xp);
    ...
    public static void addSingleCrystallization(String input, String output, int out, float xp) {
        if (!OreDictionary.getOres(output)
            .isEmpty()) {
            ItemStack o = OreDictionary.getOres(output)
                .get(0)
                .copy();
            o.stackSize = out;
            for (ItemStack stack : OreDictionary.getOres(input)) addSingleCrystallization(stack, o, xp);
        }
    }
    ...
    public static void addTransmutation(String input, String output, float xp) {
        if (!OreDictionary.getOres(output)
            .isEmpty())
            for (ItemStack stack : OreDictionary.getOres(input)) addTransmutation(
                stack,
                OreDictionary.getOres(output)
                    .get(0),
                xp);
    ...
    public static void addTransmutation(String input, String output, int out, float xp) {
        if (!OreDictionary.getOres(output)
            ...
            ItemStack o = OreDictionary.getOres(output)
                ...
                .copy();
            o.stackSize = out;
            for (ItemStack stack : OreDictionary.getOres(input)) addTransmutation(stack, o, xp);
        }
    }
    ...
    public static void addCoin(ItemStack coin) {
        if (!EngraverRecipes.instance()
            .getCoinList()
            .contains(coin))
            EngraverRecipes.instance()
                .addCoin(coin);
    ...
    public static void addEngraving(ItemStack coin, ItemEngraving engraving, float xp) {
        if (!EngraverRecipes.instance()
            .getEngravingList()
            .containsKey(engraving)
            && !EngraverRecipes.instance()
                .getEngravingList()
                .containsValue(coin))
            EngraverRecipes.instance()
                .addEngraving(coin, engraving, xp);
        else FMLLog
            .log("AbyssalCraftAPI", Level.ERROR, "This Engraving Template and/or Engraved Coin is already registered!");
    }
    ...
    public static void addEngraving(Item coin, ItemEngraving engraving, float xp) {
        if (!EngraverRecipes.instance()
            ...
            && !EngraverRecipes.instance()
                .getEngravingList()
                .containsValue(new ItemStack(coin)))
            EngraverRecipes.instance()
                .addEngraving(coin, engraving, xp);
        else FMLLog
            .log("AbyssalCraftAPI", Level.ERROR, "This Engraving Template and/or Engraved Coin is already registered!");
    ...
    public static void addMaterialization(ItemStack[] input, ItemStack output) {
        ...
        if (input.length > 0 || input == null) if (input.length <= 5) MaterializerRecipes.instance()
            .materialize(input, output);
        else FMLLog
            .log("AbyssalCraftAPI", Level.ERROR, "This Materializer recipe has more than 5 inputs! (%d)", input.length);
     ...
     * (Note: It's useless to add your entity here if it extends {@link EntityAnimal}, {@link EntityAmbientCreature},
     * {@link EntityWaterMob} or {@link EntityTameable}).
    ...
    public static class ACEntities {
        ...
        private static String[] mobNames = { "depthsghoul", "evilpig", "abyssalzombie", "Jzahar", "abygolem",
            "dreadgolem", "dreadguard", "dragonminion", "dragonboss", "shadowcreature", "shadowmonster", "dreadling",
            "dreadspawn", "demonpig", "gskeleton", "chagarothspawn", "chagarothfist", "chagaroth", "shadowbeast",
            "shadowboss", "antiabyssalzombie", "antibat", "antichicken", "anticow", "anticreeper", "antighoul",
            "antipig", "antiplayer", "antiskeleton", "antispider", "antizombie", "lessershoggoth", "shadowtitan",
            "omotholwarden", "jzaharminion", "omotholghoul", "remnant", "greaterdreadspawn", "lesserdreadbeast",
