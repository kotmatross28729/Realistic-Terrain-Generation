...
public class NecronomiconPotionAoERitual extends NecronomiconRitual {
     ...
     */
    public NecronomiconPotionAoERitual(String unlocalizedName, int bookType, int dimension, float requiredEnergy,
        boolean remnantHelp, Object potion, Object... offerings) {
     ...
     * @param offerings       Components used to perform the ritual, are consumed afterwards
     */
    public NecronomiconPotionAoERitual(String unlocalizedName, int bookType, float requiredEnergy, Object potion,
        Object... offerings) {
    ...
    public Potion getPotionEffect() {
        if (potion instanceof Potion) return (Potion) potion;
        if (potion instanceof Integer) return Potion.potionTypes[(int) potion];
        return null;
    }
    ...
    private boolean isEntityImmune(Potion potion, Entity entity) {
        ...
        try {
            ...
            result = potion == ACPotions.Coralium_plague
                && (Boolean) utilClass.getDeclaredMethod("isEntityCoralium", EntityLivingBase.class)
                    .invoke(null, (EntityLivingBase) entity)
                || potion == ACPotions.Dread_plague
                    && (Boolean) utilClass.getDeclaredMethod("isEntityDread", EntityLivingBase.class)
                        .invoke(null, (EntityLivingBase) entity)
                || potion == ACPotions.Antimatter
                    && (Boolean) utilClass.getDeclaredMethod("isEntityAnti", EntityLivingBase.class)
                        .invoke(null, (EntityLivingBase) entity);
    ...
    protected void completeRitualServer(World world, int x, int y, int z, EntityPlayer player) {
        ...
        if (!entities.isEmpty()) for (Entity entity : entities)
            if (entity instanceof EntityLiving && !entity.isDead) if (!isEntityImmune(getPotionEffect(), entity))
