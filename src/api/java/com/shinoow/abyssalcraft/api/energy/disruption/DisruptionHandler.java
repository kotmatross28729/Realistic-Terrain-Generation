...
public class DisruptionHandler {
    ...
    public void registerDisruption(DisruptionEntry disruption) {
        ...
        do {
            ...
            compare = iter.next();
            if (disruption.getUnlocalizedName()
                .equals(compare.getUnlocalizedName())) {
                FMLLog.log(
                    "DisruptionHandler",
                    Level.ERROR,
                    "Disruption Entry already registered: %s",
                    disruption.getUnlocalizedName());
                return;
            }
        } while (!disruption.getUnlocalizedName()
            .equals(compare.getUnlocalizedName()));
    }
    ...
    public void generateDisruption(DeityType deity, World world, int x, int y, int z, List<EntityPlayer> players) {
        ...
        if (deity == null) {
            for (DisruptionEntry entry : disruptions) if (entry.getDeity() == null) dis.add(entry);
        } else for (DisruptionEntry entry : disruptions)
            if (entry.getDeity() == deity || entry.getDeity() == null) dis.add(entry);

        dis.get(world.rand.nextInt(dis.size()))
            .disrupt(world, x, y, z, players);
    }
}
