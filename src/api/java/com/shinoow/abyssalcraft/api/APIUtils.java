...
public class APIUtils {
    ...
    public static boolean isCrystal(ItemStack item) {
        if (item.getItem() instanceof ICrystal) return true;
        for (ItemStack crystal : AbyssalCraftAPI.getCrystals()) return crystal.isItemEqual(item);
        return false;
    }
    ...
    public static boolean isCoin(ItemStack item) {
        for (ItemStack coin : EngraverRecipes.instance()
            .getCoinList())
            if (coin.getItem() == item.getItem() && (coin.getItemDamage() == OreDictionary.WILDCARD_VALUE
                || coin.getItemDamage() == item.getItemDamage())) return true;
        return false;
    }
    ...
    public static boolean hasNecronomicon(EntityPlayer player) {
        return player.inventory.hasItem(ACItems.necronomicon)
            || player.inventory.hasItem(ACItems.abyssal_wasteland_necronomicon)
            || player.inventory.hasItem(ACItems.dreadlands_necronomicon)
            || player.inventory.hasItem(ACItems.omothol_necronomicon)
    ...
    public static ItemStack convertToStack(Object obj) {
        if (obj == null) return (ItemStack) obj;
        else if (obj instanceof ItemStack) return ((ItemStack) obj).copy();
        else if (obj instanceof Item) return new ItemStack((Item) obj);
        else if (obj instanceof Block) return new ItemStack((Block) obj);
        else if (obj instanceof String) return OreDictionary.getOres((String) obj)
            .get(0)
            .copy();
