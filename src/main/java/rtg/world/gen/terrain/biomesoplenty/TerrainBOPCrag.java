...
public class TerrainBOPCrag extends TerrainBase {
    ...
    public float generateNoise(OpenSimplexNoise simplex, CellNoise cell, int x, int y, float border, float river) {
        ...
        simplex.riverJitter()
            .evaluateNoise((float) x / 20.0, (float) y / 20.0, jitter);
        ...
        double[] points = cell.octave(1)
            .eval((float) pX / pointWavelength, (float) pY / pointWavelength);
        float raise = (float) ((points[1] - points[0]) / points[1]);
        raise = raise * 3f;
        raise -= 0.2f;
        if (raise < 0) raise = 0;
        if (raise > 1) raise = 1;
        float topHeight = (float) (pointHeight + pointHeightVariation
            * simplex.noise((float) x / pointHeightWavelength, (float) y / pointHeightWavelength));
