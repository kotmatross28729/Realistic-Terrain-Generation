...
public class RealisticBiomeAMBase extends RealisticBiomeBase {
    ...
    protected Block witchwoodLogBlock = GameData.getBlockRegistry()
        .getObject("arsmagica2:WitchwoodLog");
    protected byte witchwoodLogMeta = (byte) 0;
    protected Block witchwoodLeavesBlock = GameData.getBlockRegistry()
        .getObject("arsmagica2:WitchwoodLeaves");
    protected byte witchwoodLeavesMeta = (byte) 0;

    public RealisticBiomeAMBase(BiomeConfig config, BiomeGenBase b, BiomeGenBase riverbiome, TerrainBase t,
        SurfaceBase s) {
            ...
            for (int i = 0; i < 256; i++) {
                if (b[i] != null) {
                    ...
                    if (biomeName == "WitchwoodForest" && biomeClass == "am2.worldgen.BiomeWitchwoodForest") {
                        amWitchwoodForest = new RealisticBiomeAMWitchwoodForest(
                            amBiome,
                            BiomeConfigAM.biomeConfigAMWitchwoodForest);
                    }
                }
            }
        }
