...
public class TerrainHLBadlands extends TerrainBase {
    ...
    public float generateNoise(OpenSimplexNoise simplex, CellNoise cell, int x, int y, float border, float river) {
        ...
        float result = riverized(bHeight + h + m + groundNoise(x, y, 2f, simplex), channelFlattening);
    ...
    // so, rather than lakes, we have a channel network
    {
        ...
        simplex.riverJitter()
            .evaluateNoise((float) x / 60.0, (float) y / 60.0, jitter);
        double pX = x + jitter.deltax() * 35f;
        double pY = y + jitter.deltay() * 35f;

        simplex.riverJitter()
            .evaluateNoise((float) x / 10.0, y / (float) 10.0, jitter);
        pX += jitter.deltax() * 3f;
        pY += jitter.deltay() * 3f;
        /*
        ...
        double[] results = simplexCell.river()
            .eval(pX / 250.0, pY / 250.0);
