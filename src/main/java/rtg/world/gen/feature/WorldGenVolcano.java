...
public class WorldGenVolcano {
    ...
    protected static Block volcanoBlock = GameData.getBlockRegistry()
        .getObject(ConfigRTG.volcanoBlockId);
    protected static byte volcanoBlockMeta = (byte) ConfigRTG.volcanoBlockMeta;
    protected static Block volcanoPatchBlock = GameData.getBlockRegistry()
        .getObject(ConfigRTG.volcanoMix1BlockId);
    protected static byte volcanoPatchBlockMeta = (byte) ConfigRTG.volcanoMix1BlockMeta;
    protected static Block volcanoPatchBlock2 = GameData.getBlockRegistry()
        .getObject(ConfigRTG.volcanoMix2BlockId);
    protected static byte volcanoPatchBlockMeta2 = (byte) ConfigRTG.volcanoMix2BlockMeta;
    protected static Block volcanoPatchBlock3 = GameData.getBlockRegistry()
        .getObject(ConfigRTG.volcanoMix3BlockId);
    ...
    private static final int lavaHeight = 138 + 3 + (ConfigRTG.enableVolcanoEruptions ? 5 : 0); // + 3 to account for
                                                                                                // lava cone tip
    ...
    public static void build(Block[] blocks, byte[] metadata, World world, Random mapRand, int baseX, int baseY,
        ...
        for (int x = 0; x < 16; x++) {
            for (int z = 0; z < 16; z++) {
                ...
                distanceEll = (float) TerrainMath.dis2Elliptic(
                    i,
                    j,
                    baseX * 16,
                    baseY * 16,
                    ...
                    simplex.octave(1)
                        .noise2(i / 250f, j / 250f) * ventEccentricity);

                // Height above which obsidian is placed
                obsidian = -5f + distanceEll;
                obsidian += simplex.octave(1)
                    .noise2(i / 55f, j / 55f) * 12f;
                obsidian += simplex.octave(2)
                    .noise2(i / 25f, j / 25f) * 5f;
                obsidian += simplex.octave(3)
                    .noise2(i / 9f, j / 9f) * 3f;

                // Make the volcanoes "mouth" more interesting
                float ventNoise = simplex.noise2(i / 12f, j / 12f) * 3f;
                ventNoise += simplex.octave(1)
                    .noise2(i / 4f, j / 4f) * 1.5f;
                ...
                if (distanceEll < ventRadius + ventNoise) {
                    ...
                    terrainHeight += simplex.octave(1)
                        .noise2(i / 112f, j / 112f) * 5.5f;
                    terrainHeight += simplex.octave(2)
                        .noise2(i / 46f, j / 46f) * 4.5f;
                    terrainHeight += simplex.octave(3)
                        .noise2(i / 16f, j / 16f) * 2.5f;
                    terrainHeight += simplex.octave(4)
                        .noise2(i / 5f, j / 5f) * 1f;
                                ...
                                if (y > obsidian) {
                                    if (distanceEll > 10) {
                                        ...
                                        if (distanceEll < 50 && isOnSurface(x, y, z, blocks)) {
                                            ...
                                            patchNoise += simplex.octave(2)
                                                .noise2(i / 30f, j / 30f) * .9;
                                            patchNoise += simplex.octave(3)
                                                .noise2(i / 5f, j / 5f) * .6;
                                            if (patchNoise > .85) {
                                        ...
                                        if (distanceEll < 75 && isOnSurface(x, y, z, blocks)) {
                                            float patchNoise = simplex.noise2(i / 10f, j / 10f) * 1.3f;
                                            patchNoise += simplex.octave(4)
                                                .noise2(i / 30f, j / 30f) * .9;
                                            patchNoise += simplex.octave(5)
                                                .noise2(i / 5f, j / 5f) * .5;
                                            if (patchNoise > .92) {
                                        ...
                                        if (distanceEll < 75 && isOnSurface(x, y, z, blocks)) {
                                            float patchNoise = simplex.noise2(i / 10f, j / 10f) * 1.3f;
                                            patchNoise += simplex.octave(6)
                                                .noise2(i / 30f, j / 30f) * .7;
                                            patchNoise += simplex.octave(7)
                                                .noise2(i / 5f, j / 5f) * .7;
                                            if (patchNoise > .93) {
                                    ...
                                    if (distanceEll < 70 + simplex.noise2(x / 26f, y / 26f) * 5) {
                                        ...
                                        float powerNoise = simplex.octave(3)
                                            .noise2(i / 40, j / 40f) * 2;
                                        if (mapRand.nextInt(
                                            1 + (int) Math.pow(
                                                Math.abs(distanceEll - (75 + simplex.noise2(x / 26f, y / 26f) * 5)),
                                                1.5 + powerNoise) + 1)
                                            == 0) {
                                            if (mapRand.nextInt(20) == 0) {
                                                ...
                                                b = Blocks.stone; // Stone so that surfacing will run (so this usually
                                                                  // becomes grass)
                                                meta = (byte) 0;
                                            }
                                        } else {
                                        ...
                                        b = Blocks.stone; // Stone so that surfacing will run (so this usually becomes
                                                          // grass)
                                        meta = (byte) 0;
                                    }
                                } else {
