...
public class DecoTree extends DecoBase {
    ...
    public int loops;
    public float strengthFactorForLoops; // If set, this overrides and dynamically calculates 'loops' based on the
                                         // strength parameter.
    public boolean strengthNoiseFactorForLoops; // If true, this overrides and dynamically calculates 'loops' based on
                                                // (noise * strength)
    public boolean strengthNoiseFactorXForLoops; // If true, this overrides and dynamically calculates 'loops' based on
                                                 // (noise * X * strength)
    ...
    public void generate(RealisticBiomeBase biome, World world, Random rand, int chunkX, int chunkY,
        ...
        if (this.allowed) {
            ...
            if (TerrainGen.decorate(world, rand, chunkX, chunkY, TREE)) {
                ...
                float noise = simplex
                    .noise2(chunkX / this.distribution.noiseDivisor, chunkY / this.distribution.noiseDivisor)
                    * this.distribution.noiseFactor + this.distribution.noiseAddend;

                int loopCount = this.loops;
                loopCount = (this.strengthFactorForLoops > 0f) ? (int) (this.strengthFactorForLoops * strength)
                    : loopCount;
                loopCount = (this.strengthNoiseFactorForLoops) ? (int) (noise * strength) : loopCount;
                loopCount = (this.strengthNoiseFactorXForLoops) ? (int) (noise * this.strengthFactorForLoops * strength)
                    : loopCount;
                for (int i = 0; i < loopCount; i++) {
                    ...
                    if (intY <= this.maxY && intY >= this.minY && isValidTreeCondition(noise, rand, strength)) {
                        ...
                        // If we're in a village, check to make sure the tree has extra room to grow to avoid corrupting
                        // the village.
                        if (hasPlacedVillageBlocks) {
                            if (!worldUtil.isSurroundedByBlock(
                                Blocks.air,
                                2,
                                SurroundCheckType.CARDINAL,
                                rand,
                                intX,
                                intY,
                                intZ)) {
