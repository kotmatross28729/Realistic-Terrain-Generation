...
public class TerrainBOPOutback extends TerrainBase {
    ...
    public TerrainBOPOutback(float minHeight, float wavelength, float hillStrength) {
        ...
        HeightEffect mesaTops = new VariableRuggednessEffect(
            new RaiseEffect(0f),
            topVariation.plus(new RaiseEffect(hillStrength))
                .plus(topBumpiness),
            0.3f,
            0.15f,
            mesaWavelength);
    ...
    public float generateNoise(OpenSimplexNoise simplex, CellNoise cell, int x, int y, float border, float river) {
        ...
        // return terrainRollingHills(x, y, simplex, river, hillStrength, maxHeight, groundNoise,
        // groundNoiseAmplitudeHills, 4f);
    }
}
