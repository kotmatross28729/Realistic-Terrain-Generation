...
public class TreeRTGRhizophoraMucronata extends TreeRTG {
    ...
    /**
     * <b>Rhizophora Mucronata (Asiatic Mangrove)</b><br>
     * <br>
     * <u>Relevant variables:</u><br>
     * logBlock, logMeta, leavesBlock, leavesMeta, trunkSize, crownSize, noLeaves<br>
     * minBranches, maxBranches, branchLength, verStart, verRand<br>
     * <br>
     ...
     *
     */
    public TreeRTGRhizophoraMucronata(int minBranches, int maxBranches, float branchLength, float verStart,
        float verRand) {
        this();
    ...
    public boolean generate(World world, Random rand, int x, int y, int z) {
        ...
        if (b != Blocks.grass && b != Blocks.dirt && b != Blocks.sand) {
            if (!(b == Blocks.water && world.getBlock(x, y - 2, z) == Blocks.sand
        ...
        if (this.trunkSize > 0) {
            for (int k = 0; k < 3; k++) {
                generateBranch(
                    ...
                    y + this.trunkSize,
                    z,
                    (120 * k) - 40 + rand.nextInt(80),
                    1.6f + rand.nextFloat() * 0.1f,
                    this.trunkSize * 2f,
                    1f,
                    true);
            }
    ...
    public void generateBranch(World world, Random rand, float x, float y, float z, double horDir, float verDir,
        float length, float speed, boolean isTrunk) {
        if (verDir < 0f) {
            verDir = -verDir;
        }
