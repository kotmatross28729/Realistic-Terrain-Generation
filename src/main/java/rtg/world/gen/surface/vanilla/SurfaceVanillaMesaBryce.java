...
public class SurfaceVanillaMesaBryce extends SurfaceBase {
    ...
    public SurfaceVanillaMesaBryce(BiomeConfig config, Block top, byte topByte, Block fill, byte fillByte,
        int grassHeight) {
    ...
    public void paintTerrain(Block[] blocks, byte[] metadata, int i, int j, int x, int y, int depth, World world,
        ...
        for (int k = 255; k > -1; k--) {
            ...
            if (b == Blocks.air) {
                ...
                if (depth > -1 && depth < 12) {
                    if (cliff) {
                        blocks[(y * 16 + x) * 256 + k] = CanyonColour.MESA_BRYCE
                            .getBlockForHeight(i, effectiveHeight, j);
                        metadata[(y * 16 + x) * 256 + k] = CanyonColour.MESA_BRYCE
                    ...
                    blocks[(y * 16 + x) * 256 + k] = CanyonColour.MESA_BRYCE.getBlockForHeight(i, effectiveHeight, j);
                    metadata[(y * 16 + x) * 256 + k] = CanyonColour.MESA_BRYCE.getMetaForHeight(i, effectiveHeight, j);
                }
            }
        }
    }
